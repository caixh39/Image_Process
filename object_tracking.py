...
class myCorrelationTracker(object):
    def __init__(self, windowName='default window', inputFile="input.mp4", outputFile="output.mp4"):
        ...
        self.video_writer = cv2.VideoWriter(outputFile, fourcc, fps, (1920, 1080))
        ...
        # 跟踪目标，实时显示
            ...
            if ret:
                if self.start_flag:  # 如果是第一帧，需要先初始化
                    ...
                    cv2.putText(save_frame, "Time: {}".format(t), (x+20, y+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
...
if __name__ == '__main__':
    file_path = "/home/cuhksz/Videos/emotion/happy.avi"
    output_path = "./processed_videos/happy/"
    testTracker = myCorrelationTracker(windowName='image', inputFile=file_path, outputFile=output_path)
    testTracker.run()
